...
async function getOverrides(menuKey, locale, sourceType, ids) {
  ...
  const rows = await q(`
    ...
    WHERE menu_key=:menuKey AND locale=:locale AND source_type=:sourceType
      AND source_id IN (${placeholders});
  `, { menuKey, locale, sourceType, ...ids });
  const map = new Map();
...
/* -------------------------------------------------------------
 * CommonJS kullanıyorsan ve "export outside module" hatası alıyorsan:
 * Aşağıdaki satırları aç, üstteki export'ları kapat.
 * -------------------------------------------------------------
 */
// module.exports = {
//   getSideMenuByKey,
