...
export async function getBlogList(locale, { categorySlug, tagSlug, page=1, limit=10 } = {}) {
  ...
  const rows = await q(
    `SELECT bp.id, bpl.slug, bpl.title, bpl.summary, m.url AS hero_url,
...
export async function getBlogPostBySlug(locale, slug) {
  const post = await q(
    `SELECT bp.id, bpl.slug, bpl.title, bpl.body_html,
            COALESCE(bp.publish_at, bp.created_at) AS dt
     FROM blog_posts bp
     JOIN blog_post_locales bpl ON bpl.post_id=bp.id AND bpl.locale=:locale
