...
export async function createPage({
  ...
  const pageSlug = slug || makeSlug(title, locale);

  const result = await tx(async (conn) => {
    // Ana sayfa kaydını oluştur
    const [pageResult] = await conn.execute(
      `INSERT INTO pages (slug, status, created_at, updated_at) VALUES (?, ?, NOW(), NOW())`,
      [pageSlug, status]
    );

    const pageId = pageResult.insertId;

    // Sayfa locale kaydını oluştur
    await conn.execute(
      `INSERT INTO page_locales (page_id, locale, slug_i18n, title, content_html, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
      [pageId, locale, pageSlug, title, safeHtml]
    );

    return { id: pageId, slug: pageSlug, title, locale };
  });
...
export async function deletePage(pageId) {
  await tx(async (conn) => {
    // Önce page_locales'den sil
    await conn.execute(`DELETE FROM page_locales WHERE page_id = ?`, [pageId]);

    // Sonra ana sayfayı sil
    await conn.execute(`DELETE FROM pages WHERE id = ?`, [pageId]);
  });
