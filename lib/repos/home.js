...
`,
  ...
  const serviceCarousel = await q(
    `SELECT i.id, il.slug, il.title
     FROM home_carousels hc
     JOIN home_carousel_items hci ON hci.carousel_id=hc.id AND hci.is_active=1
     JOIN items i ON i.id=hci.item_id
     JOIN item_locales il ON il.item_id=i.id AND il.locale=:locale
     WHERE hc.scope='service' AND hc.status='published' AND (hc.publish_at IS NULL OR hc.publish_at<=NOW())
     ORDER BY hci.sort_order`,
    { locale }
  );

  const latestPosts = await q(
    `SELECT bp.id, bpl.slug, bpl.title, COALESCE(bp.publish_at, bp.created_at) AS dt, m.url AS hero_url
     FROM blog_posts bp
