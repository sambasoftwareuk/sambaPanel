...
export async function getOtherSpareParts(slug, locale, type = "spare") {
  ...
  (
    SELECT m.url
    FROM item_media im2
    JOIN media m ON m.id = im2.media_id
    WHERE im2.item_id = i.id
    ORDER BY im2.sort_order IS NULL, im2.sort_order, im2.id
    LIMIT 1
  ) AS image_url
FROM items i
JOIN item_locales il
  ON il.item_id = i.id AND il.locale = 'tr-TR'
WHERE i.type = 'service'
  AND il.slug != 'sogutma-kulesi-otomasyon'
  AND i.status = 'published'
  AND (i.publish_at IS NULL OR i.publish_at <= NOW())
ORDER BY i.id
LIMIT 10;
