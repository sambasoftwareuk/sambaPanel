...
async function resolveSegment(locale, slug) {
  ...
  if (p.length)
    return { type: "product", product_id: p[0].id, group_id: p[0].group_id };
...
/**
 * Base: tüm yedek parçalar (label, href, sort, id)
 */
async function getAllSpareParts(locale) {
  const rows = await q(
    `
    SELECT
      sp.id,
      ...
      sp.status
    FROM spare_parts sp
    JOIN spare_part_locales spl
      ON spl.spare_part_id = sp.id AND spl.locale = :locale
    WHERE sp.status = 'published' AND spl.slug_i18n IS NOT NULL
  ...
  return rows.map((r) => ({
    ...
    type: "spare_part",
    label: r.name,
    href: `/yedek-parcalar/${r.slug_i18n}`,
    sort: r.sort_order,
    status: r.status,
    visible: true,
  }));
...
/**
 * Base: tüm hizmetler (label, href, sort, id)
 */
async function getAllServices(locale) {
  const rows = await q(
    `
    SELECT
      s.id,
    ...
    WHERE s.status = 'published' AND sl.slug_i18n IS NOT NULL
...
export async function getSideMenuForPath(path, locale = "tr-TR") {
  // Path'e göre menuKey belirle
  let menuKey = "urunler"; // default
  if (path?.includes("/hizmetler")) {
    menuKey = "hizmetler";
  } else if (path?.includes("/yedek-parcalar")) {
    menuKey = "yedek-parcalar";
  ...
  // Hizmetler için menu logic
  if (menuKey === "hizmetler") {
    // Database'den hizmetleri çek
    ...
    const items = applyOverrides(services, ov);

    return [
      {
        title: header.title || "Hizmetlerimiz",
        href: header.href || "/hizmetler",
        items: items,
      },
    ];
  }

  // Yedek Parçalar için menu logic
  if (menuKey === "yedek-parcalar") {
    // Database'den yedek parçaları çek
    const spareParts = await getAllSpareParts(locale);
    const ov = await getOverrides(
      menuKey,
      locale,
      "spare_part",
    ...
    const items = applyOverrides(spareParts, ov);

    return [
      {
        title: header.title || "Yedek Parçalar",
        href: header.href || "/yedek-parcalar",
        items: items,
      },
    ];
  }

  // Ürünler için mevcut logic
