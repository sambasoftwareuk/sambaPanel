...
export async function GET(req) {
  ...
  if (scopes.length) {
    where.push(`EXISTS (
      ...
      WHERE ms.media_id = m.id AND ms.scope IN (${scopes.map((_, i) => `:s${i}`).join(",")})
    )`);
...
export async function POST(req) {
  ...
  try {
    const out = await tx(async (conn) => {
      // media insert (IGNORE kullanmıyoruz; aynı url tekrar ediyorsa unique kısıt yoksa yeni id oluşur)
...
export async function PATCH(req) {
  ...
  try {
    const out = await tx(async (conn) => {
      ...
      const sets = [];
      const args = [];
      if (typeof url !== "undefined") { sets.push(`url=?`); args.push(url); }
      if (typeof alt_text !== "undefined") { sets.push(`alt_text=?`); args.push(alt_text); }
      if (typeof mime_type !== "undefined") { sets.push(`mime_type=?`); args.push(mime_type); }

      if (sets.length) {
        args.push(id);
        await conn.query(`UPDATE media SET ${sets.join(", ")} WHERE id=?`, args);
      }
      ...
      if (Array.isArray(remove_scopes) && remove_scopes.length) {
        await conn.query(
          `DELETE FROM media_scopes WHERE media_id=? AND scope IN (${remove_scopes.map(() => `?`).join(",")})`,
          [id, ...remove_scopes]
        );
      }

      const [[media]] = await Promise.all([
        conn.query(`SELECT id, url, alt_text, mime_type, created_at FROM media WHERE id=? LIMIT 1`, [id]),
      ]);
      return media[0];
    });
